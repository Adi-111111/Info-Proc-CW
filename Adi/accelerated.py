...
# [ADDED] Draw a shape dict received from the PYNQ
def draw_pynq_shape(canvas, shape):
    t = shape['type']
    p = shape['params']
    if t == 'circle':
        draw_circle(canvas, p['cx'], p['cy'], p['r'], thickness=3, fill=True)
    elif t == 'rectangle':
        draw_rect(canvas, [[c[0], c[1]] for c in p['corners']], thickness=3, fill=True)
    elif t == 'polyline':
...
model_path  = _os.path.join(_script_dir, "hand_landmarker.task")
...
while True:
    ...
    # Pen-up: store the stroke, send to PYNQ after pause
